...
export const useVapiFixed = (apiKey?: string): VapiHookReturn => {
  ...
  useEffect(() => {
    // Mock Vapi implementation until proper setup
    const mockVapi = {
      on: (event: string, callback: Function) => {
        console.log(`Mock Vapi event listener: ${event}`);
      },
      start: async (config: any) => {
        console.log('Mock Vapi start:', config);
        setIsConnected(true);
        setIsLoading(false);
        // Simulate AI greeting
        setTimeout(() => {
          setTranscript(prev => [...prev, "Hi! Welcome to PrepWise, your AI-powered interview assistant. I'm here to help you prepare for your next job interview. Could you tell me your name and what type of interview you're preparing for?"]);
        }, 1000);
      },
      stop: () => {
